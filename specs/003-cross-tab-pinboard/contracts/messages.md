...
|--------|---------|---------|----------|
| `PICKER_START` | Begin picker session | `url?` (http/https, optional if `tabId` set), `tabId?` (inject into this tab), `title?` (optional custom title hint), `pinId?` (when re-pinning/updating an existing pin) | `{ ok, started?, tabId?, error? }` immediately after the picker is injected or the source page is opened. |
| `PICKER_SAVE_PIN` | Content script only: save one pin during a session | From picker: `pageUrl`, `pageTitle`, `faviconUrl`, `field`: `{ label, selector, valueText }` | `{ ok, pin?, sessionComplete?, error? }` |
...
| `REFRESH_PIN` | Re-read DOM for one pin | `id` | `{ type: 'REFRESH_PIN_RESULT', id, ok, pin?, error? }` |
...
Responses to `PICKER_SAVE_PIN`, `PICKER_SESSION_END`, and `PICKER_CANCEL` are delivered via `sendResponse` on the same message.
